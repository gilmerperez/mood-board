...
function placeElementClickHandler(event) {
  if (currentElement) {
    // TODO: Explain what getBoundingClientRect() does: It returns the size of an element and its position relative to the window in pixels
    const moodBoardRect = moodBoardEl.getBoundingClientRect();

    // TODO: Explain what the following code does: It calculates the position so that the top left corner of the element aligns with the mouse click position
    ...
    // TODO: Explain why we remove the draggable class from the element: So that the element is no longer draggable after it is on the mood board
    ...
    // TODO: Explain what the `tagName` property is used for: Saves to the temporary storage based on the type of element image or text
